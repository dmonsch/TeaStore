...
public final class ServiceLoadBalancer {
	...
	/**
	 * Initializes load balancers for the target services. Queries target service
	...
	public static void preInitializeServiceLoadBalancers(Service... targetServices) {
		for (Service service : targetServices) {
			// initialize before logging in case the compiler optimizes it away when
			// log-level info is not set
	...
	private static ServiceLoadBalancer getServiceLoadBalancer(Service targetService) {
		...
		if (serviceBalancer == null || serviceBalancer.serviceServers == null
	...
	/**
	 * Gets the load balancer for a service. Initializes it with a list of know
	 * servers, if the service is not known exists.
	 * 
	...
	/**
	 * Update all load balancers for a service. Triggers Registry client to ask
	...
	/**
	 * Update all load balancers for a service with servers. Triggers Registry
	 * client to ask registry for updates.
	 * 
	...
	/**
	 * Load balances a REST operation. Automatically creates the needed load
	 * balancers, clients, etc. if needed.
	 * 
	 ...
	 * @param operation     The operation to load balance as Java8 lambda. E.g.:
	 *                      "client -> CRUDOperations.getEntity(client, id)".
	 ...
	 */
	public static <T, R> R loadBalanceRESTOperation(Service targetService, String endpointURI, Class<T> entityClass,
			Function2<Integer, RESTClient<T>, R> operation) throws NotFoundException, LoadBalancerTimeoutException {
		return getServiceLoadBalancer(targetService).loadBalanceRESTOperation(endpointURI, entityClass, operation);
	}

	private <T, R> R loadBalanceRESTOperation(String endpointURI, Class<T> entityClass,
			Function2<Integer, RESTClient<T>, R> operation) throws NotFoundException, LoadBalancerTimeoutException {
		R r = null;
		...
		try {
			...
			if (loadBalancer == null || loadBalancer.getAllServers().isEmpty()) {
				...
				ServiceLoadBalancerResult<R> slbr = LoadBalancerCommand.<ServiceLoadBalancerResult<R>>builder()
						.withLoadBalancer(loadBalancer).withRetryHandler(retryHandler).build().submit(server -> {
							EndpointClientCollection<T> endpoints = (EndpointClientCollection<T>) getEndpointClientCollection(
									endpointURI, entityClass);
							return Observable.just(ServiceLoadBalancerResult.fromRESTOperation(
									endpoints.getServerID(server), endpoints.getRESTClient(server), operation));
				...
				if (slbr.getStatusCode() == Status.REQUEST_TIMEOUT.getStatusCode()) {
					throw new LoadBalancerTimeoutException("Timout at endpoint: " + endpointURI
							+ ", with target service: " + targetService.getServiceName(), targetService);
	...
	/**
	 * Sends a multicast to all known instances of the service. Does not repeat
	 * failed sends.
	 * 
	 ...
	 */
	public static <T, R> List<R> multicastRESTOperation(Service targetService, String endpointURI, Class<T> entityClass,
	...
	/**
	 * Sends a multicast to all known instances of this service, except for the one
	 * actually sending (this instance). Does not repeat failed sends.
	 * 
	 * @param endpointURI The endpoint URI (e.g., "cache").
	 * @param entityClass The class of entities to send/receive.
	 * @param operation   The operation to load balance as Java8 lambda. E.g.:
	...
	public static <T, R> List<R> multicastRESTToOtherServiceInstances(String endpointURI, Class<T> entityClass,
			Function<RESTClient<T>, R> operation) {
		return getServiceLoadBalancer(RegistryClient.getClient().getMyService()).multicastRESTOperation(endpointURI,
				entityClass, operation, RegistryClient.getClient().getMyServiceInstanceServer());
	}
